...
class TaskSphereApp {
  ...
  setupThemeToggle() {
    ...
    // --- Hub nav theme toggle (moon/sun icon in the narrow rail) ---
    const hubThemeToggle = document.getElementById('hubNavThemeToggle');
    const hubThemeIcon  = document.getElementById('hubNavThemeIcon');

    const sunPath  = 'M12 7c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5-2.24-5-5-5zM2 13h2c.55 0 1-.45 1-1s-.45-1-1-1H2c-.55 0-1 .45-1 1s.45 1 1 1zm18 0h2c.55 0 1-.45 1-1s-.45-1-1-1h-2c-.55 0-1 .45-1 1s.45 1 1 1zM11 2v2c0 .55.45 1 1 1s1-.45 1-1V2c0-.55-.45-1-1-1s-1 .45-1 1zm0 18v2c0 .55.45 1 1 1s1-.45 1-1v-2c0-.55-.45-1-1-1s-1 .45-1 1zM5.99 4.58a.996.996 0 00-1.41 0 .996.996 0 000 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41L5.99 4.58zm12.37 12.37a.996.996 0 00-1.41 0 .996.996 0 000 1.41l1.06 1.06c.39.39 1.02.39 1.41 0s.39-1.03 0-1.41l-1.06-1.06zm-1.06-10.96a.996.996 0 000 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41l-1.06-1.06a.996.996 0 00-1.41 0zm-11.31 11.31a.996.996 0 000 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41l-1.06-1.06a.996.996 0 00-1.41 0z';
    const moonPath = 'M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z';

    const syncHubIcon = () => {
      if (!hubThemeIcon) return;
      const isLight = document.body.classList.contains('light-theme');
      hubThemeIcon.innerHTML = `<path d="${isLight ? moonPath : sunPath}"/>`;
      if (hubThemeToggle) hubThemeToggle.title = isLight ? 'Switch to Dark Mode' : 'Switch to Light Mode';
    };
    syncHubIcon();

    if (hubThemeToggle) {
      hubThemeToggle.onclick = (e) => {
        e.stopPropagation();
        const isLight = document.body.classList.contains('light-theme');
        const newTheme = isLight ? 'dark' : 'light';
        localStorage.setItem('tasksphere_theme', newTheme);
        updateThemeButtons(newTheme);
        syncHubIcon();
      };
    }

    // --- Hub home button: close hub and go back to dashboard ---
    const hubHomeBtn = document.getElementById('hubNavHome');
    if (hubHomeBtn) {
      hubHomeBtn.onclick = () => {
        this.switchRoute('DASHBOARD');
      };
  ...
  setupSecuritySettings() {
    ...
    const customSoundInput = document.getElementById('customSoundFileInput');
    if (customSoundInput) {
      customSoundInput.onchange = (e) => {
        ...
        if (file.size > maxBytes) {
          alert('Chime audio file size must be under 4MB.');
        ...
        reader.onload = (event) => {
          try {
            ...
            localStorage.setItem('tasksphere_custom_sound_data', base64);
            const customFileName = document.getElementById('customSoundFileName');
            if (customFileName) {
              customFileName.textContent = file.name;
            }
            // Automatically play new sound as test
            this.playNotificationSound('custom');
          } catch (err) {
            console.error('Failed to store custom notification sound:', err);
            alert('Failed to load custom chime audio. Local storage limit might be exceeded.');
          }
        };
  ...
  async openSecurityModal() {
    ...
    const soundSelect = document.getElementById('settingsSoundSelect');
    if (soundSelect) {
      const activeSound = localStorage.getItem('tasksphere_chat_sound') || 'minimal';
      soundSelect.value = activeSound;
      const uploadRow = document.getElementById('customSoundUploadRow');
      if (uploadRow) {
        uploadRow.style.display = (activeSound === 'custom') ? 'flex' : 'none';
      }
      const customFileName = document.getElementById('customSoundFileName');
      if (customFileName) {
        const savedData = localStorage.getItem('tasksphere_custom_sound_data');
        if (savedData) {
          customFileName.textContent = 'Custom Chime Loaded';
        } else {
  ...
  playNotificationSound(soundTheme) {
    try {
      ...
      if (theme === 'custom') {
        const customData = localStorage.getItem('tasksphere_custom_sound_data');
        if (customData) {
          const audio = new Audio(customData);
          audio.volume = 0.5;
          audio.play().catch(e => console.warn('[AUDIO-CUSTOM] Playback blocked or failed:', e));
        }
        return;
      }
      ...
      const runSynthesis = () => {
        ...
        if (theme === 'cyber') {
          ...
          // Digital Chime (Double-Synth Beep)
          ...
          // Classic Bell (Multi-Tone Chord)
          const frequencies = [587.33, 880.00, 1174.66, 1567.98];
          const gains = [0.08, 0.05, 0.03, 0.02];
          const durations = [0.6, 0.4, 0.3, 0.2];
