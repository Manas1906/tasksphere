        ...
        return args -> {
            try {
                ...
                if ("PostgreSQL".equalsIgnoreCase(dbName)) {
                    log.info("[DB-INIT] Creating feature_toggles table if not exists for PostgreSQL...");
                    jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS feature_toggles (" +
                    ...
                    log.info("[DB-INIT] Creating time_logs table if not exists...");
                    jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS time_logs (" +
                            "id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, " +
                    ...
                    log.info("[DB-INIT] Creating task_dependencies table if not exists...");
                    jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS task_dependencies (" +
                            "id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, " +
                            "task_id BIGINT NOT NULL, " +
                    ...
                    log.info("[DB-INIT] Creating kanban_columns table if not exists...");
                    jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS kanban_columns (" +
                            "id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, " +
                    ...
                    log.info("[DB-INIT] Seeding default kanban columns if not present...");
                    jdbcTemplate.execute("INSERT INTO kanban_columns (column_key, column_name, position, color) VALUES ('TODO','Backlog',0,'#818cf8') ON CONFLICT (column_key) DO NOTHING;");
                    jdbcTemplate.execute("INSERT INTO kanban_columns (column_key, column_name, position, color) VALUES ('IN_PROGRESS','Work In Progress',1,'#34d399') ON CONFLICT (column_key) DO NOTHING;");
                    jdbcTemplate.execute("INSERT INTO kanban_columns (column_key, column_name, position, color) VALUES ('REVIEW','Quality Assurance',2,'#fbbf24') ON CONFLICT (column_key) DO NOTHING;");
                    jdbcTemplate.execute("INSERT INTO kanban_columns (column_key, column_name, position, color) VALUES ('DONE','Closed Scope',3,'#4ade80') ON CONFLICT (column_key) DO NOTHING;");
                    log.info("[DB-INIT] Kanban columns ready.");
